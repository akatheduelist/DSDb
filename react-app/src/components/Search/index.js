              ...
              <div className='search-result-card-container' key={result.id}>
                <div className='search-result-card-left'>
                  <span className='search-result-card-title'>
                    <Link to={`/vehicles/${result.id}`} target='_blank'>
                      {result.year} {result.make} {result.model}
                    </Link>
                  </span>
                  <span className='search-result-card-info mid-bold'>
                    Dougscore: {result.dougscore.dougscore_total}/100
                  </span>
                  <span className='search-result-card-info mid-bold'>
                    Daily Total: {result.dougscore.daily_total}/50
                  </span>
                  <span className='search-result-card-info mid-bold'>
                    Weekend Total: {result.dougscore.weekend_total}/50
                  </span>
                  <span className='search-result-card-info mid-bold'>
                    YouTube Review:{' '}
                    <Link
                      to={{ pathname: result.dougscore.video_link }}
                      target='_blank'
                    >
                      {result.dougscore.video_link}
                    </Link>
                  </span>
                </div>
